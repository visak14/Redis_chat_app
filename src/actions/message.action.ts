...
type sendMesssageActionArgs = {
     content: string;
     messageType: "text" | "image",
     receiverId: string ;
}
export async function sendMesssageAction({content, messageType, receiverId}: sendMesssageActionArgs) {
    
    const {getUser} = getKindeServerSession();
    const user = await getUser();

    if(!user) return {return: false, message:"user not authenticated"};
